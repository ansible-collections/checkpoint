...
DOCUMENTATION = """module: cp_mgmt_service_group_facts
short_description: Get service-group objects facts on Check Point over Web Services
  API
description:
...
options:
  ...
  show_as_ranges:
    description:
    - When true, the service group's matched content is displayed as ranges of port
      numbers rather than service objects.<br />Objects that are not represented using
      port numbers are presented as objects.<br />The 'members' parameter is omitted
      from the response and instead the 'ranges' parameter is displayed.
    type: bool
  details_level:
    description:
