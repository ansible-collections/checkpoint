...
import pytest
from ansible_collections.checkpoint.checkpoint.tests.unit.modules.utils import (
    set_module_args,
    exit_json,
    fail_json,
    AnsibleExitJson,
...
from ansible_collections.checkpoint.checkpoint.plugins.modules import (
    cp_mgmt_service_icmp6,
)

OBJECT = {"name": "Icmp1", "icmp_type": 5, "icmp_code": 7}

CREATE_PAYLOAD = {"name": "Icmp1", "icmp_type": 5, "icmp_code": 7}

UPDATE_PAYLOAD = {"name": "Icmp1", "icmp_type": 45, "icmp_code": 13}

OBJECT_AFTER_UPDATE = UPDATE_PAYLOAD

DELETE_PAYLOAD = {"name": "Icmp1", "state": "absent"}
...
class TestCheckpointServiceIcmp6(object):
    ...
    def connection_mock(self, mocker):
        connection_class_mock = mocker.patch(
            "ansible_collections.checkpoint.checkpoint.plugins.module_utils.network.checkpoint.checkpoint.Connection"
        )
